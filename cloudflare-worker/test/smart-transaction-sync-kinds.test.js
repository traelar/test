...
function mutation(kind, recordId) {
  return {
    mutationId: `stage-a-${kind}-${recordId}`,
    kind,
    recordId,
    baseVersion: 0,
    deleted: false,
...
test('merchant profiles smart rules and review resolutions are household sync kinds', async () => {
  const sync = createSyncService(new MemorySyncStore(), { now: () => new Date('2026-09-13T00:00:00Z') });
  const mutations = [
    mutation('merchant_profile', 'merchant-1'),
    mutation('smart_transaction_rule', 'rule-1'),
    mutation('review_resolution', 'fingerprint-1')
  ];

  const result = await sync.sync(context, { sinceEventId: 0, mutations });

  assert.deepEqual(
    result.applied.map((row) => [row.kind, row.recordId]),
    [
      ['merchant_profile', 'merchant-1'],
      ['smart_transaction_rule', 'rule-1'],
      ['review_resolution', 'fingerprint-1']
    ]
  );
});
