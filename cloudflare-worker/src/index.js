...
async function route(request, env) {
  ...
  if (request.method === 'POST' && url.pathname === '/api/plaid/link-token') {
    const result = await plaidPost(env, '/link/token/create', {
      client_name: 'BillNest', language: 'en', country_codes: ['US'], products: ['transactions'],
      android_package_name: 'com.baylee.billnest', user: { client_user_id: 'billnest-personal' }
    });
  ...
  if (request.method === 'POST' && url.pathname === '/api/plaid/exchange') {
    ...
    const result = await plaidPost(env, '/item/public_token/exchange', { public_token: body.publicToken });
    ...
    await env.DB.prepare(`
      ...
      VALUES (?1, ?2, ?3, ?4)
      ON CONFLICT(item_id) DO UPDATE SET access_token_enc = excluded.access_token_enc, label = excluded.label
  ...
  if (request.method === 'GET' && url.pathname === '/api/plaid/accounts') {
    const rows = await env.DB.prepare('SELECT item_id, access_token_enc, label, created_at FROM plaid_items ORDER BY created_at ASC').all();
    const accounts = [];
  ...
  if (request.method === 'GET' && url.pathname === '/api/plaid/items') {
    const rows = await env.DB.prepare('SELECT item_id, label, created_at FROM plaid_items ORDER BY created_at ASC').all();
    return json({ items: (rows.results || []).map((row) => ({ itemId: row.item_id, label: row.label, createdAt: row.created_at })) });
  }
...
export default {
  async fetch(request, env) {
    try { return await route(request, env); }
      ...
      return json({ error: error.message || 'Server error', plaidErrorCode: error.plaid?.error_code || null }, error.status || 500);
    }
  }
};
