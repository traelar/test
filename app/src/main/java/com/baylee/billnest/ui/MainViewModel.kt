...
class MainViewModel(val repo: BillRepository) : ViewModel() {
    ...
    fun setAccountPreference(p: AccountPreference) = repo.setAccountPreference(p)
    fun moveAccount(id: String, delta: Int) = repo.moveAccount(id, delta)
    fun addReservedFund(fund: ReservedFund) = repo.addReservedFund(fund)
    fun updateReservedFund(fund: ReservedFund) = repo.updateReservedFund(fund)
